 ...
 *
 * As the design has it: the address and what it holds, the two things you
 * can do with it, which network it is on, the way to the invite, what the
 * exchange calls this account, and at the bottom the only destructive thing
 * on the screen.
 *
      ...
      >
        ...
        <Card style={{ gap: theme.space.s3 }} testID="wallet-card">
          ...
          <View style={{ flexDirection: 'row', gap: theme.space.s2 }}>
            <Button testID="add-funds" title="Add funds · demo" small style={{ flex: 1 }} onPress={() => router.push('/deposit')} />
            <Button testID="withdraw" title="Withdraw" variant="outline" small style={{ flex: 1 }} onPress={() => router.push('/withdraw')} />
          </View>
        </Card>

        <Card style={{ gap: theme.space.s3 }} testID="network-card">
          <Text variant="small" style={{ fontSize: theme.type.t2xs }}>Network</Text>
          <View style={{ flexDirection: 'row', gap: theme.space.s2 }}>
            <View style={{ flex: 1 }}>
              <Chip label="Practice" on center testID="network-testnet" />
            </View>
            ...
            <View style={{ flex: 1, opacity: 0.4 }}>
              <Chip label="Real money" center testID="network-mainnet" onPress={() => router.push('/mainnet')} />
            </View>
          </View>
          <Text variant="small" style={{ fontSize: theme.type.t2xs }}>
            Practice money, Monad testnet. Real money is not open yet; each network has its own account and balance.
          </Text>
        </Card>
        ...
        >
          <View style={{ gap: 2, flexShrink: 1 }}>
            <Text variant="bodyStrong" style={{ fontSize: theme.type.tSm }}>Invite friends</Text>
        ...
        {t.state ? (
          <FadeIn>
            <Text variant="small" style={{ fontSize: theme.type.t2xs }} testID="exchange-account">
