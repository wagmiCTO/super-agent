...
func NewSignals(v venue.Adapter, log *slog.Logger) *Signals {
	...
	return &Signals{venue: v, log: log, now: time.Now, macross: make(map[string]*strategy.MACross)}
}
...
func (s *Signals) feed(ctx context.Context, symbol string, sig *strategy.MACross) {
	cfg := strategy.DefaultMACross(symbol)
	for ctx.Err() == nil {
		if err := s.seed(ctx, symbol, sig, cfg); err != nil {
...
func (s *Signals) seed(ctx context.Context, symbol string, sig *strategy.MACross, cfg strategy.MACrossConfig) error {
	now := s.now()
